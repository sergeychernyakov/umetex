...
class TextTranslator:
    ...
    def find_phrases_in_combined_text(self, texts: List[str]) -> Dict[str, Optional[str]]:
        """
        Find all phrases from TranslationPhrase in the combined text of shuffled_texts.
        ...
        # Найти все фразы, которые содержатся в объединенном тексте
        for phrase in TranslationPhrase.objects.filter(target_language=self.translation_language):
            if phrase.source_phrase in combined_text:
                found_phrases[phrase.source_phrase] = phrase.translated_phrase
                logger.debug(f"Found phrase in combined text: {phrase.source_phrase} -> {phrase.translated_phrase}")
    ...
    def translate_texts(self, texts: List[str]) -> List[Optional[str]]:
        ...
        message = (
            "The following is a list of text segments extracted from a document. "
        ...
        if found_phrases:
            message += "Found phrases and their translations:\n"
